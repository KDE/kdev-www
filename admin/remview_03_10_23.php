...
///////////////////////////////// S E T U P ///////////////////////////////////
   ...
   include ("../../access.inc");
   $login=$k_login;         // Login & password for access to this programm.
   $pass=$k_password;       // Example: $login="MyLogin"; $pass="MyPaSsWoRd";
                            // Type 'login=false' for disable authorization.
   unset($k_login);
   unset($k_password);

   // Type list of your(allow) hosts. All other - denied.
   // Example: $host_allow=array("127.0.0.*","localhost")
   $host_allow=array("*");
   //$host_allow=array("134.169.117.7","82.82.188.212","127.0.0.1");
...
///////////////////////////////////////////////////////////////////////////////
   ...
   if (!preg_match($s,getenv("REMOTE_ADDR")) && !preg_match($s,gethostbyaddr(getenv("REMOTE_ADDR")))) 
      exit("<h1><a href=http://php.spb.ru/remview/>phpRemoteView</a>: Access Denied - your host ".getenv("REMOTE_ADDR")." is not allowed</h1>\n");
            ...
            case "t": 
               ...
               case "file":
                  echo jsval("<a href=$self/".urlencode($k)."?&c=v&fnot=1&ftype=3&d=".
                     urlencode($d)."&f=".urlencode($k)." class=sym title='view/download'>v</a> ".
                     "<a href=$self?&c=i&d=".urlencode($d)."&f=".urlencode($k)." class=sym title='details/extras'>+</a>");
                  break;
               case "link":
...
4. <input type=checkbox name=df4 value=1 id=df4>
  <label for=df4>".mm("convert file name to lowercase")."</label><br>
...
case "fileupload_submit":
   ...
   if ($fname=="") 
     exit("".mm("You must check 'create file name automatic' OR type a file name!").""); 
   if (isset($df4)) $fname=strtolower($fname);

   echo "Temp file (on server): ".$HTTP_POST_FILES['userfile']['tmp_name']."<br>";
   ...
   if ($HTTP_POST_FILES['userfile']['size'] == 0){
     echo 'Sorry the file was <font color=red><b>probably too big</b></font> (usualy the servers have a 2Mb limit)<br>';
   }
...
<P>";
   ...
   if (!isset($shell)) {
      $shell="#".mm("example (remove comments '#')").": \n\n#tail /var/log/httpd/kdevelop.httpd.access_log\n\n#tail /var/log/httpd/kdevelop.httpd.error_log\n\n#/home/smeier/public_html/www/bin/inform_translators.php\n\n#ps -ax\n\n#uname -a\n\ncd ../mediawiki/ && /usr/bin/php maintenance/update.php --quick\n\ncd ../mediawiki/ && /usr/bin/php maintenance/refreshLinks.php\n\ncd ../mediawiki && /usr/bin/php maintenance/deleteIdleUsers.php --verbose";
      $skipshell=1;
   }
